-- Momentum Rebound - Detailed Analysis
...
-- Calculate average volume over last 20 days from history_ohlc
avg_volume_20d AS (
    SELECT
        code AS security_code,
    ...
    WHERE
        volume IS NOT NULL
        AND trade_date >= (SELECT MAX(trade_date) FROM public.history_ohlc) - INTERVAL '30 days'
    GROUP BY
        code
    HAVING
        COUNT(*) >= 15  -- Relaxed from 20 to 15 days
),
...
WHERE
    -- Show stocks with at least long lower shadow and liquidity (your 20 candidates)
